---
name: Ambia
description: Use when coordinating solar interconnection applications, logging utility approvals (Part 1/Part 2/PTO), navigating Salesforce/TaskRay projects, updating IX task owners, extracting log reviews, or preparing utility portal submissions - coordinates end-to-end interconnection lifecycle across Salesforce, TaskRay, and external utility portals with autonomous task execution but mandatory user approval before submission.
---

# Ambia Skill - Main Workflow Guide

**Version:** 1.4 (Solar IX Task Structure Added) | **Status:** ✅ Production Ready | **Last Updated:** 2025-11-25
...
- **"What fields are on IX task pages?"** → See **Solar Interconnection Task Structure & Data Model** (line 562)
- **"What are the task dependencies?"** → See **Task Hierarchy and Dependencies** (line 564)
- **"What date fields should I track?"** → See **Key Date Fields on IX Task Pages** (line 579)
- **"What's in the IXP1 Details section?"** → See **IXP1 Details Section Fields** (line 592)
- **"The database is stale, should I refresh?"** → See **project-data-mcp** skill "Stale Data Handling" section
...
## Solar Interconnection Task Structure & Data Model

### Task Hierarchy and Dependencies

The interconnection process uses TaskRay to manage tasks with a **Finish-To-Start** dependency model:

**Task Flow Pattern**:
```
Request IX Part 1 → Receive and Process IX Part 1 (0 lag)
Prepare IX Part 1 / Prepare IX Part 2
Request IX Part 2 → Receive and Process IX Part 2 (0 lag)
```

**Dependency Rules**:
- "Receive and Process IX Part 1" cannot begin until "Request IX Part 1" is completed
- Dependencies use 0 lag time (no buffer between tasks)

### Key Date Fields on IX Task Pages

When extracting or updating IX tasks, these date fields are critical:

| Field | Purpose | Location |
|-------|---------|----------|
| **Open Date** | When the task was opened/created | Information section |
| **Most Recent Open Date** | When the task was last reopened | Information section |
| **Task Due** | Target completion date | Information section |
| **Estimated End Date** | Projected end date shown in header | Header section |
| **Last Reviewed** | Timestamp of last review activity | Information section |
| **Actual Completion Date** | When task was actually completed | Information section |

### IXP1 Details Section Fields

The "IXP1 Details" section on Part 1 tasks tracks application lifecycle:

| Field | Purpose | Example Value |
|-------|---------|---------------|
| **IXP1 Application Prepared** | Date application documents were prepared | 2024-06-15 |
| **IXP1 Application Submitted** | Date application was submitted to utility | 2024-06-20 |
| **IXP1 Application Approved** | Date utility approved the application | 2024-11-12 |
| **IXP1 Application Fee** | Fee amount paid | $50.00 |
| **IXP1 Application REF#** | Utility reference number(s) | DER-40976 \| New: DER-73012 |
| **IXP1 Application Expiration** | Date the approval expires | 2025-11-12 |
| **IXP1 Application Rejected** | Date application was rejected (if applicable) | — |

**Note**: Similar fields exist for IXP2 (Part 2) tasks with "IXP2" prefix.

### Activity Log/Timeline Structure

The Activity Timeline (right panel on IX task pages) contains chronological log review entries with these subjects:

| Subject | Purpose |
|---------|---------|
| **Interconnection: Payment** | Payment-related updates |
| **Interconnection: Submitted** | Application submission confirmations |
| **Interconnection: Approved** | Approval notifications |
| **Interconnection: Follow Up** | Follow-up actions needed |
| **Interconnection: Utility Communication** | Communications with utility |

**Each entry shows**:
- Subject/type
- Assigned reviewer (e.g., "Ellen Castillo had a task")
- Date (grouped by month: November 2025, June 2024, etc.)

### IX Task Detail Page Sections

When navigating IX task pages, expect these sections:

1. **Header**: Task name, Project link, Estimated End Date, List status, Owner
2. **Information**: List status, Task Name, Task Group (IXP1/IXP2), Owner, Days Open
3. **Task Fields**: IXP1 Rejection Reason (if rejected)
4. **IXP1/IXP2 Details**: All application date milestones and reference numbers
5. **Utility Details**: Utility Company name (e.g., "Ameren Illinois"), Account Number, Meter Number
6. **Additional Information**: Opportunity link, State, Jurisdiction, Annual Consumption, System Size, Gross Price
7. **System Information**: Role, Created By, Last Modified By
8. **Scope of Work**: Description with completion instructions
9. **TaskRay Dependencies**: Shows predecessor tasks and dependency type
10. **Files for Parent Opportunity**: Attached documents (PDFs, engineering files)
...
| **Holding** | Awaiting information or external action |
| **Reopened** | Previously closed but returned for additional work |
| **Not Applicable** | Task doesn't apply to this project |
| **Completed** | Task finished |
| **Inactive** | Task not currently active |

### Workflow Process Flow

**Part 1 Flow**:
```
Request IX Part 1 → [Utility Reviews] → Receive and Process IX Part 1 → IXP1 Approved
```

**Part 2 Flow**:
```
Prepare IX Part 2 → Request IX Part 2 → [Utility Reviews] → Receive and Process IX Part 2 → IXP2 Approved
```

**Common Blockers** (shown by "Holding" status):
- Missing signatures
- Missing photos
- Missing placards
- CAD updates needed
- Missing portal access
- Missing HOI (Homeowner Insurance)

### Navigation Notes for IX Tasks

- Tasks are linked to **Opportunities** (the customer/project record)
- Tasks are linked to **Projects** (the TaskRay project container)
- The **"Log Review"** button creates new activity entries
- **"View All"** expands full activity history
- Files are attached at the **parent Opportunity level**
- **IX Tickets** section tracks any support tickets related to interconnection

**Complete Lifecycle Tracking**: This structure allows tracking the complete lifecycle of utility interconnection applications from initial submission through final approval, with full audit trail of all communications and status changes.
...
## Data Freshness Check (Required Before All Workflows)

**BEFORE executing any interconnection workflow**, verify database freshness:

1. **Call `get_database_status`** from project-data-mcp MCP tool
2. **Check response fields**:
   - `age_hours`: Time since last database update
   - `is_stale`: Boolean flag indicating staleness
   - `warning`: Staleness warning message (if present)
3. **Follow decision tree** from project-data-mcp skill "Stale Data Handling" section:
   - `age_hours < 4`: FRESH - Proceed with workflow
   - `age_hours 4-24`: WARNING - Proceed with caution, note staleness
   - `age_hours > 24`: STALE - Stop workflow, recommend refresh
4. **If refresh needed**:
   - Pause current workflow
   - Guide user through refresh procedure (see project-data-mcp skill "Database Refresh Procedures")
   - Wait for user confirmation that refresh completed
   - Re-check `get_database_status` to verify freshness
   - Resume workflow with fresh data

**Critical Workflows** (require age_hours < 4):
- Application submissions to utility portals
- Logging approvals (Part 1, Part 2, PTO)
- Compliance checks and final validations
- Bulk reporting (15+ projects)

**Non-Critical Workflows** (can proceed with age_hours < 24):
- Quick project lookups
- Status checks
- Document gathering (non-submission)

**For complete refresh instructions**, see **project-data-mcp skill** → "Database Refresh Procedures" section.
...
- Agent has a customer identifier (e.g., address "2633 JORDAN") or a link/reference
- **Database freshness verified** (age_hours < 4 for approval logging)
...
**For detailed IX task structure, fields, and data model**, see **Solar Interconnection Task Structure & Data Model** section above.
...
**Status**: ✅ PRODUCTION READY (v1.4 - Solar IX Task Structure Added)
...
**Recent Changes (v1.4)**:
- Added comprehensive Solar Interconnection Task Structure & Data Model section:
  - Task hierarchy and Finish-To-Start dependencies (0 lag time)
  - Key date fields on IX task pages (Open Date, Most Recent Open Date, Task Due, etc.)
  - IXP1/IXP2 Details section fields (Application Prepared, Submitted, Approved, Fee, REF#, etc.)
  - Activity Log/Timeline structure with subjects (Payment, Submitted, Approved, Follow Up, Utility Communication)
  - IX Task Detail Page sections (10 key sections documented)
  - Task status values and meanings (Open, Holding, Reopened, etc.)
  - Workflow process flow (Part 1 and Part 2 flows with common blockers)
  - Navigation notes for IX tasks
- Consolidated duplicate information (removed redundant Task Taxonomy and State Model sections)
- Updated Quick Index with references to new Solar IX structure
- Optimized YAML frontmatter with "Use when..." description pattern
