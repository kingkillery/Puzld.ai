...
**Version:** 1.2 (Optimized - RED Phase Fixes Applied) | **Status:** ✅ Production Ready | **Last Updated:** 2025-11-13
...
1. **Portal URL Extraction**:
   ...
   - If not found: Document as **Assumption** - "Portal URL not found in guide. Will request from user before portal navigation."

2. **Application Form/Checklist Extraction**:
   ...
   - If not found: Use generic checklist (utility bill, insurance, one-line diagram, site plan) and document as **Assumption**

3. **Submission Template/Process Extraction**:
   ...
   - If not found: Document as **Assumption** - "Submission process not detailed. Will navigate portal UI manually."
...
4. **Fee Schedule Extraction**:
   ...
   - If not found: Document as **Assumption** - "Fee information not found. Will verify in portal before submission."

5. **Credentials and Authentication Info**:
...
6. **When Extraction Fails After All Steps**:
   - If a required item is not found after searching guide + fallback locations:
     - Document the gap: "**Assumption** - [Item] not found in [Guide Name] or `/docs/references/`. Searched keywords: [list]. Checked sections: [list]."
     - Add to retrieval plan: "Will request [Item] from user OR navigate portal to discover manually."
     - Proceed to Phase 4 if you have ≥70% of required information

**Extraction Success Criteria**: You should have identified at least 70% of: portal URL, document checklist, fee schedule, and submission workflow. If below 70%, proceed to Refinement Pass (see budget section below).
...
**Proceed if ≥70% of required docs pass.** Otherwise create **Request** with precise missing items and where you looked. Store the Attachment Map in the answer and a TaskRay note.
...
**Budget Rule**: One parallel batch per phase; **max 2 tool calls** before acting. Escalate once only if blockers remain after plan validation.

**Completeness Threshold**: Proceed at **70% completeness**. Record **Assumption** lines for gaps with source and retrieval plan.

**Decision Tree for Budget vs Completeness Conflicts**:

```
After 2 tool calls, evaluate completeness:

├─ Path A: 2 calls achieved ≥70% completeness
│  └─ ACTION: Proceed to next phase. Document assumptions for the 30% gap.
│
├─ Path B: 2 calls achieved <70% completeness AND you are blocked from portal navigation
...
│  │           ├─ If now ≥70% OR blocker resolved → Proceed to next phase
│  │           └─ If still <70% AND still blocked → Create **Follow Up** task with:
│  │              - Missing items list
...
│
└─ Path C: 2 calls achieved <70% completeness BUT you are NOT blocked from portal navigation
...
3. At least 70% of required information
...
│  ├─ Phase 1: Training Guide Search → Google connector
│  ├─ Phase 2: Project Context Collection → Salesforce + Drive
│  ├─ Phase 3: Resource/Portal Prep → Extract URLs, fetch credentials (use concrete extraction steps)
│  ├─ Phase 4: Consolidation → Build master resource list
...
**Phase 3: Resource and Portal Prep**
...
### Assumption Line
`Assumption — {statement}. Source — {source_ref}.`
...
- **70% completeness threshold**: Proceed with assumptions when 70% complete
- **Document assumptions**: Always record gaps with Assumption lines
...
**Status**: ✅ PRODUCTION READY (Optimized - RED Phase Fixes Applied)
...
**Changes**: Applied 6 RED phase fixes (ambiguous scope, concrete extraction, decision tree, page confirmation, approval flow, workflow selection)
